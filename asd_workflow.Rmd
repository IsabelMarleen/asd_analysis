...
I use brain-map.org sNucseq data (cells sorted by cortical layers) to find genes
discriminating between L1, L2/3 and L5/6. They did not sort layer 4 at all, 
perhaps because it is thin and hard to dissect?


All analysis I did in brainmap.R (currently on my laptop only), here are the
top 20 markers for each contrast:
```{r}
l56_lt_l23 <- c("MYZAP", "SCUBE1", "FIBCD1", "LOC441178", "RPS24P10", "BAMBI",
                "MROH2A", "LOC101927605", "LRAT", "IL26", "PRKRIRP4",
                "SAMD9", "LOC105379291", "LOC105377419", "LOC105373172",
                "LOC101927439", "SHB", "CLCP2", "LOC728283", "LINC00890") 
l23_lt_l56 <- c("LOC105377171", "LOC101928409", "CTBP2P5", "LOC105378598",
                "ATP5HP2", "LOC105370785", "LOC105371526", "LOC101930665",
                "LOC105375415", "LYN", "YWHAQP9", "MTHFD2P1", "LOC105370416",
                "LAMP5", "LOC105378751", "KCNJ5", "LOC100420940", "LOC105376771",
                "CCDC88C", "C6orf141")

# L1 vs L2/3:  much more genes upregulated in L2/3 than other way around
l1_lt_l23 <- c("SH3BP4", "ITIH4-AS1", "NDNF", "PA2G4P1", "LOC105371143",
"LOC105379336", "LOC101927895", "LOC100129603", "ZSCAN5D", "SP110", "GALNT8",
"NR2F2", "MIR670HG", "LOC105371160", "MAGOH3P", "LOC105371320", "PTPRH",
"HNRNPA3P13", "AATBC", "LOC105378912")
l23_lt_l1<-c("CH25H", "ANXA1", "SLC38A11", "RDH10", "CLEC14A", "LOC649024",
             "LOC100506368", "RDH12", "FAM83D", "GAL", "CDK1", "COBLL1", "SGCG",
             "SMPX", "NGB", "LOC340017", "LOC105378762", "LINC00592", "INSM2", "CYP27B1")

# L1 vs L5/6: more genes upregulated in L5/6 than other way around
l56_lt_l1 <- c("LOC101927281", "SH2D1B", "ANXA1", "CH25H", "CYP26A1", "LOC649024",
"LOC101927605", "LOC105376592", "GAL", "LOC105375792", "NXT2", "LOC101927389",
"SLC38A11", "BAMBI", "LOC100506368", "SMIM24", "SMPX", "RDH12", "VAV1",
"LOC100419870", "LOC105377785", "OLFML2B", "TMEM173", "IQCF3", "LOC100507651",
"INSM2", "LOC105369438", "COBLL1", "CLEC14A", "LOC105378762", "LOC728819",
"LOC105378969", "LOC101928622", "CYBRD1", "LOC102724899", "LOC101929294", "CDK1",
"LOC105371497", "LOC105374852", "CABP7", "H2AFJ", "CHRNA1", "SLC5A7", "COL18A1-AS1",
"PTX3", "ANXA3", "TMEM132C", "LOC105374615", "SLC7A3", "CENPK")

l1_lt_l56 <- c("LOC100533708", "LOC105377171", "LOC100421595", "NDNF",
 "LOC105377845", "LOC101930665", "RPS2P43", "SDC4", "HIST1H2BM", "LOC105370785",
 "TNFRSF10D", "LOC105378021", "PA2G4P1", "LOC100129603", "CD53", "LOC105378835",
 "LOC105378409", "LOC105373485", "PAX2", "LMOD2", "KLF17", "HIST2H2BB",
 "LOC105376779", "LOC101928880", "MEIS1-AS3", "CCDC88C", "MTND5P15", "ZNF619P1",
 "LAMP5", "ITIH4-AS1", "NPM1P45", "LOC105377097", "PROX1", "LOC105370416", "THEGL",
 "XAF1", "C1QL4", "CLCA4", "LOC283332", "HIST2H2BA", "LOC105372909", "RIN3",
 "LOC105373351", "LOC284379", "ZRF1PS", "MTHFD2P1", "LOC105377901", "SOX1",
 "CERKL", "INHBA")
```

```{r}
library(UpSetR)
upset(fromList(list(l56_lt_l23=l56_lt_l23,
                    l56_lt_l1=l56_lt_l1,
                    l23_lt_l1=l23_lt_l1,
                    l23_lt_l56=l23_lt_l56,
                    l1_lt_l23=l1_lt_l23, 
                    l1_lt_l56=l1_lt_l56
                    )), nsets = 6, order.by = "freq")

layermarkers <- list(l56_lt_l23=l56_lt_l23,
 l56_lt_l1=l56_lt_l1,
 l23_lt_l1=l23_lt_l1,
 l23_lt_l56=l23_lt_l56,
 l1_lt_l23=l1_lt_l23, 
...
layermarkers <- unlist(layermarkers)[ unlist(layermarkers) %in% rownames(norm_counts)]


poisson_vmr <- mean(1/sfs[celltypes_louv == "Neuron_excit"])
gene_means <- rowMeans( norm_counts[, celltypes_louv == "Neuron_excit"] )
gene_vars <- rowVars_spm( norm_counts[,celltypes_louv == "Neuron_excit"] )
is_expressed <- colSums( Tcounts[celltypes_louv == "Neuron_excit",] != 0 ) > 100
is_informative <- gene_vars/gene_means > 1.5 * poisson_vmr  &  is_expressed
plot(gene_means, gene_vars/gene_means, pch=".", log = "xy")
points(gene_means[is_informative], (gene_vars/gene_means)[is_informative], pch=".", col = "red" )


layermarkers <- layermarkers[ layermarkers %in% rownames(norm_counts)[is_informative] ]
# duplicated markers can exist if one gene is a DEG in two different contrasts:
layermarkers <- unique(layermarkers)

cors_layerm <- cor(as.matrix(t(sqrt(
  norm_counts[layermarkers,
              celltypes_louv == "Neuron_excit" &
                 dblts_perc < 3/50  & nn_inothercluster < 1]))))
cors_layerm[cors_layerm > .9] <- NA



library(pheatmap)
library(RColorBrewer)
pheatmap(cors_layerm,
         color = colorRampPalette(rev(brewer.pal(n = 7, name =
  "RdYlGn")))(19),
  breaks = c(seq(min(cors_layerm, na.rm = T), -1e-10, length.out = 10),
             seq(1e-10, max(cors_layerm, na.rm = T), length.out = 10)   ),
...
Conclusion: 
The correlations are not convincing, I doubt the markers I got out of
brain-map.org are particularly helpful in telling regions apart.
I drop it for now and continue with something else.
